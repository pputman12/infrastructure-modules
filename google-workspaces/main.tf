...
locals {
  ...
  suspended_users = [ for user in data.googleworkspace_users.existing-workspace-users : user if !contains(keys({for users in googleworkspace_user.users : user.primary_email => user }), user)]
