...
locals {
  ...
  suspended_users = [ for user in data.googleworkspace_users.existing-workspace-users.users : user if !contains(keys({for assigned_user in googleworkspace_user.users : assigned_user.primary_email => assigned_user }), user.primary_email) && user.isadmin_false]
